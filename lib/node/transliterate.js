...
var _languagesConfig = require('../../data/languages-config');

var _languagesConfig2 = _interopRequireDefault(_languagesConfig);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

let charmap = {};
let languagesConfig = _languagesConfig2.default;
...
const transliterate = (sourceStr, options) => {
  let opt = null;
  if (!options) {
    opt = (0, _utils.mergeOptions)(defaultOptions, configOptions);
  } else {
    // check for a 'lang' option and apply the language configurations (if it exists)
    opt = (0, _utils.mergeOptions)(defaultOptions, languagesConfig[options.lang] || configOptions);

    const restOfOptions = (0, _utils.mergeOptions)(defaultOptions, options);
    ...
    opt = (0, _utils.mergeOptions)(opt, restOfOptions);
  }
